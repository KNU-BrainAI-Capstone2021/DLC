...
def detect_cloth(capture_img):
    ...
    for out in outs:
        for detection in out:
            ...
            if confidence > 0.3: #confidence 값이 1에 가까우면 정확도가 높아지고, 0에 가까우면 정확도가 떨어지지만 검출되는 개체 수가 많아진다.
                # Object detected 
    ...
    p=''
    #중복박스제거코드            
    indexes = cv2.dnn.NMSBoxes(boxes, confidences, 0.4, 0.1)
    ...
    print('원하는 옷의 종류를 선택하여 주십시요:\n'+p)
    num=int(input())
    category=str(classes[class_ids[num]])
    
    position_x=boxes[num][0]-20
    position_y=boxes[num][1]-20
    position_w=boxes[num][0]+boxes[num][2]+20
